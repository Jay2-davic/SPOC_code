...
### Cleaning EIS file on 04252024

cleaning_04252024_files <- function(user_name) {   
  ...
  new_files <- orig_files |>
    dplyr::mutate(
      new_filename = stringr::str_replace(orig_filename, '/pin(\\d+)_column(\\d+)_row(\\d+)_(\\d+)_(\\d+)_(\\d+)_(\\d+)_(\\d+)_(\\d+)', ''),
  ...
  readr::write_csv(
    new_files,
    glue::glue('C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\\mpr files\\data_dictionary\\04252024_Board104_vary_0to20wtNaTFSI_10wtAE380_database_dictionary.csv)')
  )
  return(new_files)
}

cleaning_04252024_files(user_name)

rm(cleaning_04252024_files)
