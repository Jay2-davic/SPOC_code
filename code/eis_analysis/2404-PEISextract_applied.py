...
'''
sys.path.append('Users\cooper94\Documents\Git\23-spoc_code\EIS_Analysis\PEISextract.py') #folder where PEISextract is located
...
def load_dir(directory):
  ...
  Inputs:
    directory = path to the csv file of mpr files

  Outputs:
    df = dataframe with paths of MPR files that have been marked as done with cleaning process
  '''                                                                     f'C:\\Users\\{OUN}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\EIS_full_files.csv'
  df = pd.read_csv(directory)
...
def apply_randles_fits_wrapper(n, R0_guess, R1_guess, Wo1_0_guess, Wo1_1_guess, C1_guess, head_count, save_file):
  ...
  Inputs:
    ...
    head_count = number of data points
...
def nyquist_GUI(n, R0_guess, R1_guess, Wo1_0_guess, Wo1_1_guess, C1_guess, head_count, save_file):
  '''
  Opens an interactive GUI to investigate how 
  ...
  Outputs:
    A GUI window with adjustable sliders and a visual representation of the Nyquist plot
...
>>>>>>> main
  ...
  # %%
nyquist_GUI(0, 1800, 20000, 1, 1, -2, 90, False)
# %%
