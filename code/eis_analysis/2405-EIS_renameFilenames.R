...
df_PE_combined <- df_PE_combined |>
  dplyr::mutate(
    ...
    EIS_mpr = stringi::stri_replace_all_regex(EIS_mpr,
                                              ...
                                              'none'),
    EIS_mpr = stringi::stri_replace_all_regex(EIS_mpr,
                                              '/High-Throughput Polymer Electrolytes DIW - General',
                                              '/General - High-Throughput Polymer Electrolytes DIW'),
  ...
  dplyr::filter(!stringr::str_detect(EIS_mpr, 'dictionary')) |>
  # combines with the full data
  dplyr::rename(orig_filename = EIS_mpr) |>
  dplyr::full_join(EIS_files) |>
  dplyr::select(-mpr_files) |>
  dplyr::rename(legacy_mpr = orig_filename,
                mpr_files = new_filename) |>
  dplyr::mutate(legacy_mpr = stringi::stri_replace_all_regex(legacy_mpr,
                                                             '/High-Throughput Polymer Electrolytes DIW - Shared DocumentsC:/',
                                                             'C:/')
  )
