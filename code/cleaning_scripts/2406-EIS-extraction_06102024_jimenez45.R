...
cleaning_06102024_files <- function(user_name) {   
  ...
  folder_struct <-
    glue::glue('C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\\20240610_9_PEGMEA_1_PEGDA_10_Aerosil380_0to20_NaTFSI_1600ppm_Inhibitor')
  # extracts the file paths of EIS files
  orig_files <- EIS_finder(folder_struct)
  # formats the names
  new_files <- orig_files |>
    dplyr::mutate(
      new_filename = stringr::str_replace(orig_filename, 'RUN(\\d+)/2024_(\\d+)_(\\d+)_(\\d+)_(\\d+)_(\\d+)_A(\\d+)_', ''),
  ...
  readr::write_csv(
    new_files,
    glue::glue('C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\\mpr files\\data_dictionary\\20240610_9_PEGMEA_1_PEGDA_10_Aerosil380_0to20_NaTFSI_1600ppm_Inhibitor_database_dictionary.csv')
  )
  return(new_files)
}

cleaning_06102024_files(user_name)

rm(cleaning_06102024_files)
