...
### Cleaning EIS file on 08142024

cleaning_08152024_files <- function(user_name) {   
  ...
  folder_struct <-
    glue::glue('C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\\20240815_9_PEGMEA_1_PEGDA_0to10_Aerosil380_0_NaTFSI_100ppm_Inhibitor')
  # extracts the file paths of EIS files
  orig_files <- EIS_finder(folder_struct)
  # formats the names
  new_files <- orig_files |>
    dplyr::mutate(
      new_filename = stringr::str_replace(orig_filename, 'RUN(\\d+)/2024_(\\d+)_(\\d+)_(\\d+)_(\\d+)_(\\d+)_([a-zA-Z])(\\d+)', ''),
      ...
      new_filename = dplyr::case_when(
        stringr::str_detect(new_filename, '_A(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_A(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_A\\1_')),
        stringr::str_detect(new_filename, '_B(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_B(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_B\\1_')),
        stringr::str_detect(new_filename, '_C(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_C(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_C\\1_')),
        # different because file DUSB0 exists
        stringr::str_detect(new_filename, '_D(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_D(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_D\\1_')),
        stringr::str_detect(new_filename, '_E(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_E(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_E\\1_')),
        stringr::str_detect(new_filename, '_F(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_F(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_F\\1_')),
        stringr::str_detect(new_filename, '_G(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_G(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_G\\1_')),
        stringr::str_detect(new_filename, '_H(\\d)_') == TRUE ~ stringr::str_replace(new_filename, '_H(\\d)_', glue::glue('_{wt_pct_vary_additive}_Aerosil380_{wt_pct_vary_salt}_{salt}_H\\1_')),
        TRUE ~ new_filename
      ),
    ) |>
  ...
  readr::write_csv(
    new_files,
    glue::glue('C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Database of polymer electrolytes\\mpr files\\data_dictionary\\20240815_9_PEGMEA_1_PEGDA_0to10_Aerosil380_0_NaTFSI_100ppm_Inhibitor_database_dictionary.csv')
  )
  return(new_files)
}

cleaning_08152024_files(user_name)

rm(cleaning_08152024_files)
