...
folder_destination <- glue::glue(
  'C:\\Users\\{user_name}\\OneDrive - LLNL\\General - High-Throughput Polymer Electrolytes DIW\\Jay\\forJune\\LiTFSI\\'
)
...

{
  final_db_outlier <- final_db |>
    ...
    dplyr::filter(
      print_type == 'Coin cell' & !fcomp_additive_wt_pct %in% c(2.9, 5, 5.7, 10, 7.1) |
        print_type == 'PCB print' & !fcomp_additive_wt_pct %in% c(1.4, 2.9, 5, 5.7, 10, 7.1)) |>
    dplyr::select(
      fcomp_formulation,
